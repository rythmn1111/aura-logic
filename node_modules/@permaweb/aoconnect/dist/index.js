...
function requestWith2(env) {
  return (fields) => {
    return of({ path: `/~${fields.device ?? env.device}`, ...fields, method: fields.method ?? method }).chain(verifyInput).chain(dispatch(env)).map(logResult(env, fields)).map(transformToMap).bimap(errFrom, identity).toPromise();
  };
}
