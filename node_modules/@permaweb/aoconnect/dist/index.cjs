...
// src/client/hb.js
var reqFormatCache = {};
function toSigBaseArgs({ url, method: method2, headers, includePath = false }) {
  ...
  return {
    ...
    request: { url, method: method2, headers: { ...Object.fromEntries(headers) } }
  };
}
...
function requestWith(args) {
  ...
  return async function(fields) {
    const { path: path2, method: method2, ...restFields } = fields;
    ...
    try {
      ...
      if (signingFormat === "ANS-104") {
        ...
        fetch_req = { ...signedRequest, body: req.body, path: path2, method: method2 };
      }
...
function toANS104Request(fields) {
  ...
  const dataItem = {
    ...
    tags: (0, import_ramda2.keys)(
      (0, import_ramda2.omit)(
        [
          "Target",
          "target",
          "Anchor",
...
var createLogger = (name = "@permaweb/aoconnect") => {
  ...
  logger.tap = (note, ...rest) => (0, import_ramda4.tap)((...args) => logger(note, ...rest, ...args));
  return logger;
};
...
function requestWith2(env) {
  return (fields) => {
    return of({ path: `/~${fields.device ?? env.device}`, ...fields, method: fields.method ?? method }).chain(verifyInput).chain(dispatch(env)).map(logResult(env, fields)).map(transformToMap).bimap(errFrom, import_ramda5.identity).toPromise();
  };
}
